...
def load_subjects():
	...
	if os.path.isdir("/tmp/subjects") == False:
		print("Error: no subjects folder found")
		exit(1)
	
	count = 0
	for folder in os.listdir("/tmp/subjects"):
		if folder.startswith("level"):
			level = int(folder.split("level")[1])
			subjects[level] = []
			for subject in os.listdir("/tmp/subjects/" + folder):
				subjects[level].append(Subject("/tmp/subjects/" + folder + "/" + subject))
